...
export function SyncButton({ onPress, status, deviceCount, desktopConnected }: SyncButtonProps) {
  const colors = useColors();

  const getStatusColor = () => {
    if (desktopConnected) return '#10b981';
    switch (status) {
      case 'syncing':
  ...
  const getStatusIcon = () => {
    if (desktopConnected) return 'check-circle';
    switch (status) {
      case 'syncing':
        return 'sync';
      case 'connected':
  ...
  const getLabel = () => {
    if (desktopConnected) return 'Desktop Online';
    switch (status) {
      case 'syncing':
