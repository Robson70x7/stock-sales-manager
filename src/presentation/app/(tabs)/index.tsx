...
import {
  ...
  ScrollView, Animated, PanResponder, Dimensions, Modal, Platform
} from 'react-native';
...
export default function HomeScreen() {
  ...
  const { data: monthSales = [] } = useSalesByMonth(currentYear, currentMonth);
  ...
  const filteredItems = useMemo(() => {
    ...
    return result;
  }, [summaryItems, search, activeTagFilter, statusFilter, filterPayment, sortBy, tags]);
              ...
              {item.installmentInfo && (
                <>
                  <Text style={[styles.installmentBadge, { color: colors.muted }]}>
                    {' '}({item.installmentInfo.isEntry ? `Entrada · ${getPaymentTypeLabel((item.installmentInfo.entryPaymentType || item.paymentType) as PaymentType)}` : `${item.installmentInfo.number}/${item.installmentInfo.total}`})
                  </Text>
                </>
              )}
            </Text>
