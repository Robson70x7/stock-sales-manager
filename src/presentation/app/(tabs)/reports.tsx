...
import { formatCurrency, getMonthName, getPaymentTypeLabel, isInMonth } from '@shared/lib/utils';
...
export default function ReportsScreen() {
  ...
  const reportData = useMemo(() => {
    ...
    const totalReceived = filteredSales.reduce((sum, s) => {
      const installmentsPaid = s.installments.filter(i => i.status === 'paid').reduce((a, i) => a + i.amount, 0);
      return sum + installmentsPaid + (s.entryAmount || 0);
    }, 0);
  ...
  }, [sales, startYear, startMonth, endYear, endMonth, filterPayment, filterClientId, filterTagId]);
